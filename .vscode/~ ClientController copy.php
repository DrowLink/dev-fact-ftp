...
class ClientController extends BaseController
{
    ...
    public function getClientListCRM() //Recoge clientes de CipherCRM
    {
        $responseCRM = Http::withHeaders([
            'X-Api-Key' =>  '1000dd7c41e9f6b7b448f9988fb85589'
            ])->get('https://dev.crm.cipher.icu/api/v1/Contact');

        if ($responseCRM->ok()) {
            $clientList = $responseCRM->json();
            dd($clientList); //Renderiza en json, esta data debe trabajarse ahora
            return $clientList;
        } else {
            dd('No se pudo obtener clientes');
            return null;
        }
    }

    public function getClientListInvNinja() //Recoge clientes de Aca, InvoiceNinja
    {
        $responseInvNinja = Http::withHeaders([ //Debe haber otra manera que no sea haciendo GET a este mismo sistema
            'X-Api-Key' => 'iF27QP9jvNSuDSOnWcEqT0Ipq6yDfykkfkZz6ssnivjonQazIdDugGpo7SkwygHS'
            ])->get('https://dev.fact.cipher.icu/api/v1/clients');

        if ($responseInvNinja->ok()) {
            $clientListInvNinja = $responseInvNinja->json();

            foreach
            $id_number = $_GET["id_number"]; //Intentar obtener id number de cada cliente [!!!] 
            dd($clientListInvNinja); //Renderiza en json, esta data debe trabajarse ahora
            return $clientListInvNinja;
        } else {
            dd('No se pudo obtener clientes');
            return null;
        }
    ...
    public function compareDataClientLists()
    {
        $idnumber = '2389977'; //Este valor debe ser relativo, evaluar cada Contact de ESPOCRM, para ver si ya existe aca, o sino, crearlo.

        $findClient = Client::firstOrNew([
            'idnumber' => $idnumber
        ]);

        if (isset($clientList) && isset($clientListInvNinja)) {
            
        } else {
            dd('Hubo un problema con las variables');
        }
    }
}
